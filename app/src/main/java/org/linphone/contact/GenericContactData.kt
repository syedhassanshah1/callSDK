...
open class GenericContactData(private val sipAddress: Address) : ContactDataInterface {
    ...
    init {
        BaseApplication.coreContext.contactsManager.addListener(contactsUpdatedListener)
        contactLookup()
    }

    open fun destroy() {
        BaseApplication.coreContext.contactsManager.removeListener(contactsUpdatedListener)
    }

    private fun contactLookup() {
        contact.value = BaseApplication.coreContext.contactsManager.findContactByAddress(sipAddress)
    }
}
